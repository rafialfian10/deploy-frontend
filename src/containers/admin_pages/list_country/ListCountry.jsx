    ...
    try {
      ...
      if( messageError.name === "") {
        ...
        formData.append('name', form.name);
         ...
         // patch
        let response = await API.patch(`/country/${countryId}`, formData, config);
                ...
                <h2 className="title-country">Update country</h2>
                  <Form onSubmit={(e) => handleUpdateCountry.mutate(e)}>
                    <Form.Group className="form-group" controlId="formBasicEmail"> 
                      ...
                      <Form.Control type="text" name="name" onChange={(e) => handleChange(e, 'name')} value={valueCountry.name} />
                      {error.name && <Form.Text className="text-danger">{error.name}</Form.Text>}
                    </Form.Group>
                    <div className="btn-country">
                      <Button variant="primary" className="btn-cancel" onClick={handleCloseModal}>cancel</Button>
                      <Button variant="primary" type="submit" className="btn-add-country" onClick={() => handleUpdateCountry.mutate}>Submit</Button>
                    </div>
                  </Form>
            </Modal.Body>
          </Modal>
